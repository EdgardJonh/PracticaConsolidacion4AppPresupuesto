...
function enviarPresupuesto(){
    ...
    let formu = document.getElementById('formul')
    formu.reset()
}
...
function pintarGastosEliminar(){
    ...
    for(let j=0; j<limiteEliminar; j++){
        ...
        let unBotonNuevo = document.createElement('input');
        ...
        unParrafoTextoNuevo.setAttribute('class', 'border-bottom ')
        unParrafoValorNuevo.setAttribute('class', 'border-bottom ')
        unBotonNuevo.type = 'button'
        unBotonNuevo.setAttribute('class', 'fa-solid fa-trash-can')
        unBotonNuevo.style.color = '#0c6dfd'
...
function enviarGasto(){
    ...
    if (valorCajaGasto ==''  && valorCantidadGasto =='' ) {
        alert('Ingresa datos en el Campo Nombre Gastos: ""')
    }else{
